...
class VirtualBotEnv:
  def __init__(self, bot_id, bot_cfg, width=960, height=800):
    ...
    # Initialize logger for this bot
  ...
  def _load_questions(self):
    ...
    if not self.dataset_path or not os.path.exists(self.dataset_path):
      self.logger.error("QUESTIONS_LOAD_FAILED", f"Файл с вопросами не найден: {self.dataset_path}")
      return None, None

    start_row, end_row = self.row_range
    self.logger.info("QUESTIONS_LOAD", {
    ...
    try:
      ...
      self.logger.success("QUESTIONS_LOADED", {
        "count": len(pdf),
        "first_index": pdf.index[0]
      })
      return pdf, start_row
    except Exception as e:
      self.logger.error("QUESTIONS_LOAD_ERROR", str(e))
      import traceback
      traceback.print_exc()
      return None, None
  ...
  def save_verified_json(self, json_data: dict):
    ...
    try:
      ...
      if uid is None:
        self.logger.log_json_failed("Не удалось получить UID вопроса")
        return None

      global_idx = self.cur_global_idx
      self.logger.debug("JSON_SAVE_START", {
        "cur_global_idx": global_idx,
        "uid": uid
      })
      ...
      self.logger.log_json_saved(filepath, uid, global_idx)
      return filepath

    except Exception as e:
      self.logger.log_json_failed(str(e), self.get_cur_question_uid())
      return None

  def advance_question(self):
    """Advances to the next question index"""
    if self.questions is not None:
      old_idx = self.cur_global_idx
      start_row, end_row = self.row_range
      ...
      uid = self.get_cur_question_uid()
      self.logger.log_question_advance(old_idx, self.cur_global_idx, uid)
  ...
  def start(self, url):
    ...
    self.logger.log_browser_action("PROFILE_PREPARED", True, {"url": url})
    self.logger.info("BOT_STARTING", {"display": self.display, "url": url})
    ...
    self.logger.log_browser_action("BROWSER_STARTED", True, {
      "vnc_port": vnc_port,
      ...
      try:
        ...
        if time.time() - last_display_check > 30:
          ...
          if result.returncode != 0 and "Failed" in result.stderr:
            self.logger.log_browser_action("DISPLAY_CHECK", False, {
              "display": self.display,
              "error": result.stderr
            })
          last_display_check = time.time()
        ...
        if t_type is None or val is None:
          self.logger.action_failed("ACTION_QUEUE", f"Получены None в action_queue (action #{action_count})")
          continue

        # Отладочное логирование каждого 10-го действия
        if action_count % 10 == 0:
          self.logger.debug("ACTION_PROGRESS", {
        ...
        if t_type == 'click':
          if not isinstance(val, (list, tuple)) or len(val) != 2:
            self.logger.action_failed("CLICK", f"Неверный формат click: {val}")
            continue
          result = subprocess.run(
          ...
          self.logger.log_click(tuple(val), success)
          if not success:
            self.logger.log_browser_action("CLICK_ERROR", False, {"error": result.stderr})
          ...
          success = result.returncode == 0
          self.logger.action(f"KEY_{key_name.upper()}" if success else None, {"key": key_name})
          if not success:
            self.logger.log_browser_action("KEY_ERROR", False, {
              "key": key_name,
              "error": result.stderr
            })
          ...
          success = result.returncode == 0
          self.logger.action(f"HOTKEY_{key_combo.upper()}" if success else None)
          if not success:
            self.logger.log_browser_action("HOTKEY_ERROR", False, {
              "hotkey": key_combo,
              "error": result.stderr
            })
          ...
          success = result.returncode == 0
          self.logger.action(f"TYPE_{len(val)}_chars" if success else None)
          if not success:
            self.logger.log_browser_action("TYPE_ERROR", False, {"error": result.stderr})
        ...
        self.logger.error("EXECUTOR_ERROR", str(e))
        import traceback
        traceback.print_exc()
        continue
  ...
  def stop(self):
    self.logger.info("BOT_STOPPING", {"reason": "User requested shutdown"})
